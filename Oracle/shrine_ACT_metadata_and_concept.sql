...
set echo on;
-------------------------------------------------------------------------------
-- Create "&&heron_ont_schema".ont tables
-------------------------------------------------------------------------------
whenever sqlerror continue;
--select '||''''|| 'drop table "&&heron_ont_schema".'||c_table_name ||''''|| ';' sql from SHRINE_ONT_ACT.table_access order by sql;
drop table "&&heron_ont_schema".ACT_CPT_PX_2018AA;
-- Following table ACT_HCPCS_PX_2018AA is using metadata approach,but does not require changes.
drop table "&&heron_ont_schema".ACT_HCPCS_PX_2018AA;
drop table "&&heron_ont_schema".ACT_ICD10CM_DX_2018AA;
drop table "&&heron_ont_schema".ACT_ICD10PCS_PX_2018AA;
drop table "&&heron_ont_schema".ACT_ICD9CM_DX_2018AA;
drop table "&&heron_ont_schema".ACT_ICD9CM_PX_2018AA;
drop table "&&heron_ont_schema".ACT_LOINC_LAB_2018AA;
-- Following tables are using metadata approach,but require changes.
-- drop table "&&heron_ont_schema".ACT_MED_ALPHA_V2_121318;
--drop table "&&heron_ont_schema".ACT_MED_VA_V2_092818;
drop table "&&heron_ont_schema".NCATS_DEMOGRAPHICS;
drop table "&&heron_ont_schema".NCATS_ICD10_ICD9_DX_V1;
drop table "&&heron_ont_schema".NCATS_LABS;
drop table "&&heron_ont_schema".NCATS_VISIT_DETAILS;
whenever sqlerror exit sql.sqlcode;

--select '||''''|| 'create table "&&heron_ont_schema".'||c_table_name ||' as select * from SHRINE_ONT_ACT.'|| c_table_name ||''''||' ;' sql from SHRINE_ONT_ACT.table_access order by sql;
create table "&&heron_ont_schema".ACT_CPT_PX_2018AA as select * from SHRINE_ONT_ACT.ACT_CPT_PX_2018AA;
-- Following table ACT_HCPCS_PX_2018AA is using metadata approach,but does not require changes.
create table "&&heron_ont_schema".ACT_HCPCS_PX_2018AA as select * from SHRINE_ONT_ACT.ACT_HCPCS_PX_2018AA;
create table "&&heron_ont_schema".ACT_ICD10CM_DX_2018AA as select * from SHRINE_ONT_ACT.ACT_ICD10CM_DX_2018AA;
create table "&&heron_ont_schema".ACT_ICD10PCS_PX_2018AA as select * from SHRINE_ONT_ACT.ACT_ICD10PCS_PX_2018AA;
create table "&&heron_ont_schema".ACT_ICD9CM_DX_2018AA as select * from SHRINE_ONT_ACT.ACT_ICD9CM_DX_2018AA;
create table "&&heron_ont_schema".ACT_ICD9CM_PX_2018AA as select * from SHRINE_ONT_ACT.ACT_ICD9CM_PX_2018AA;
create table "&&heron_ont_schema".ACT_LOINC_LAB_2018AA as select * from SHRINE_ONT_ACT.ACT_LOINC_LAB_2018AA;
-- Following tables are using metadata approach,but require changes.
-- create table "&&heron_ont_schema".ACT_MED_ALPHA_V2_121318 as select * from SHRINE_ONT_ACT.ACT_MED_ALPHA_V2_121318;
--create table "&&heron_ont_schema".ACT_MED_VA_V2_092818 as select * from SHRINE_ONT_ACT.ACT_MED_VA_V2_092818;
--create table "&&heron_ont_schema".NCATS_DEMOGRAPHICS as select * from SHRINE_ONT_ACT.NCATS_DEMOGRAPHICS;
create table "&&heron_ont_schema".NCATS_ICD10_ICD9_DX_V1 as select * from SHRINE_ONT_ACT.NCATS_ICD10_ICD9_DX_V1;
create table "&&heron_ont_schema".NCATS_LABS as select * from SHRINE_ONT_ACT.NCATS_LABS;
create table "&&heron_ont_schema".NCATS_VISIT_DETAILS as select * from SHRINE_ONT_ACT.NCATS_VISIT_DETAILS;


-------------------------------------------------------------------------------
-- update "&&heron_ont_schema".ont tables which are using metdatada approach
-------------------------------------------------------------------------------
-- for medication, look at shrine_act_medication_mapping.sql

create table "&&heron_ont_schema".ncats_demographics
as
with mto1
as
(
...
-------------------------------------------------------------------------------
DELETE from 
"&&heron_ont_schema"."HERON_TERMS"
where C_FULLNAME = '\i2b2\Demographics\LESS_THAN_10\'
;
-- TERM_ID need to be max+1 eg. select max(term_id)+1 from "&&heron_ont_schema".HERON_TERMS;
INSERT INTO "&&heron_ont_schema"."HERON_TERMS" (C_HLEVEL, C_FULLNAME, C_NAME, C_SYNONYM_CD, C_VISUALATTRIBUTES, C_BASECODE, C_FACTTABLECOLUMN, C_TABLENAME, C_COLUMNNAME, C_COLUMNDATATYPE, C_OPERATOR, C_DIMCODE, M_APPLIED_PATH, UPDATE_DATE, DOWNLOAD_DATE, IMPORT_DATE, SOURCESYSTEM_CD)
...
from "&&heron_ont_schema".HERON_TERMS
where C_FULLNAME = '\i2b2\Demographics\LESS_THAN_10\'
) ib
...
(
select C_BASECODE, C_FULLNAME , C_NAME , UPDATE_DATE , DOWNLOAD_DATE,sourcesystem_cd, C_DIMCODE
from "&&heron_ont_schema".NCATS_DEMOGRAPHICS
union all
--select C_BASECODE, C_FULLNAME , C_NAME , UPDATE_DATE , DOWNLOAD_DATE,sourcesystem_cd, C_DIMCODE
--from "&&heron_ont_schema".NCATS_ICD9_DIAG_HERON
--union all
--select C_BASECODE, C_FULLNAME , C_NAME , UPDATE_DATE , DOWNLOAD_DATE,sourcesystem_cd, C_DIMCODE
--from "&&heron_ont_schema".NCATS_ICD9_PROC_HERON
--union all
--select C_BASECODE, C_FULLNAME , C_NAME , UPDATE_DATE , DOWNLOAD_DATE,sourcesystem_cd, C_DIMCODE
--from "&&heron_ont_schema".NCATS_ICD10_ICD9_DX_V1_HERON
--union all
--union all
--select C_BASECODE, C_FULLNAME , C_NAME , UPDATE_DATE , DOWNLOAD_DATE,sourcesystem_cd, C_DIMCODE
--from "&&heron_ont_schema".ncats_labs
select C_BASECODE, C_FULLNAME , C_NAME , UPDATE_DATE , DOWNLOAD_DATE,sourcesystem_cd, C_DIMCODE
from "&&heron_ont_schema".ncats_visit_details
union all
select C_BASECODE, C_FULLNAME , C_NAME , UPDATE_DATE , DOWNLOAD_DATE,sourcesystem_cd, C_DIMCODE
from "&&heron_ont_schema".ACT_MED_VA_V2_092818
union all
select C_BASECODE, C_FULLNAME , C_NAME , UPDATE_DATE , DOWNLOAD_DATE,sourcesystem_cd, C_DIMCODE
from "&&heron_ont_schema".ACT_MED_ALPHA_V2_121318
union all
select C_BASECODE, C_FULLNAME , C_NAME , UPDATE_DATE , DOWNLOAD_DATE,sourcesystem_cd, C_DIMCODE
from "&&heron_ont_schema".ACT_HCPCS_PX_2018AA
) ib
where ib.c_basecode is not null
group by ib.c_basecode, ib.c_fullname
     , update_date, download_date, sysdate, sourcesystem_cd
